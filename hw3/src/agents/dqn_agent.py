...
class DQNAgent(nn.Module):
    ...
    def get_action(self, observation: np.ndarray, epsilon: float = 0.0) -> int:
        ...
        if np.random.rand() < epsilon:
            ...
            # choose action as argmax of qa_values from current network
            with torch.no_grad():
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            if self.use_double_q:
                ...
                next_action = None
                # for each batch, find the max predicted future reward across possible actions
                next_q_values = torch.max(next_qa_values, dim=1).values

            assert next_q_values.shape == (batch_size,), next_q_values.shape

            # using Bellman equation, reward 
            done_values = done.float()
            target_values = reward + self.discount * (1 - done_values) * next_q_values
        ...
        # selects the q_values for the current actions taken
        q_values = qa_values.gather(1, action.unsqueeze(1)).squeeze(1)  # (batch_size,)
