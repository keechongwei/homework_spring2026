...
class SoftActorCritic(nn.Module):
    def __init__(
        ...
        if self.auto_tune_temperature:
            ...
            self.log_alpha = torch.tensor(np.log(temperature),device=ptu.device, requires_grad=True)
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        # return -action_distribution.log_prob(action_distribution.rsample()).mean(dim=-1)
        # ENDTODO
    ...
    def actor_loss_reparametrize(self, obs: torch.Tensor):
        ...
        q_min = q_values.min(dim=0).values   # (batch_size,)
        loss = -q_min.mean()
        #  loss = -q_values.mean(dim=0).sum
        # ENDTODO
    ...
    def update_alpha(self, log_prob: torch.Tensor):
        ...
        log_prob = log_prob.sum(dim=-1) if log_prob.ndim > 1 else log_prob
