...
class IQLAgent(nn.Module):
    ...
    def iql_expectile_loss(
        ...
        # adv is defined as q - v
        weight = torch.where(adv > 0, expectile, 1 - expectile)
        return (weight * adv.square()).mean()
    ...
    def update_v(
        ...
        # expectile loss is computed using q -v 
        # if q- v > 0 , expectile loss returns expectile * (q - v)^2 (due to underestimating)
        # for L(V), we want to return expectile * (q-v)^2 when underestimating 
        v = self.value(observations).squeeze(-1)
        with torch.no_grad():
    ...
    def update_q(
        ...
        with torch.no_grad():
            next_v = self.value(next_observations).squeeze(-1)
            targets = rewards + self.discount * (1 - dones) * next_v

        loss = (q-targets.unsqueeze(0)).square().mean()
    ...
    def update_actor(
        ...
        with torch.no_grad():
            ...
            v = self.value(observations).squeeze(-1)                   # [B]
            adv = q - v                                                # [B]

            weights = torch.exp(self.alpha * adv)
